...
DebugMode = 'DEBUG&IllCond';        % Set this to 'DEBUG' for a fast aproximate output, for complete soundfield contruction replace with ''
...
Starting_No_of_Pw   =   80;
Finishing_Frequency = 8000; %Hz
Finishing_No_of_Pw   =  80;
...
for w = 1:length(Weights)
    ...
    n=fprintf('%.2f%% \n\tRemaining: %d mins %.0f secs \n\tTotal: %d mins %.0f secs\n\tExpected Finish: %s\n', ratio * 100, floor(tRem/60), rem(tRem,60), floor(tTot/60), rem(tTot,60), datestr(datetime + seconds(tRem),'dd-mmm-yyyy hh:MM:SS AM'));
end

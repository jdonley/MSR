function SR_SYSTEM = OculusPres_System_B()
...
publication_info.DocumentPath = 'tex\latex\OculusPres';
publication_info.FigureName = 'IEEE_Trans2016_C';
publication_info.FigureTitle = '';
...
publication_info.axis_aspect_ratio = [1 0.8]; %Single axis aspect ratio [width height]
publication_info.axes_gap = [0.75 0.5]; %Gap between axes [gap_height gap_width] %centimeters
...
publication_info.axes_NumTicks = [4 5]; % Number of ticks [NumXticks NumYticks];
publication_info.axes_limitBufs = [0.02 0.02]; % axis limits buffer in percentage [width, height]
publication_info.sigRounding = 2; % number of significant figures rounding
publication_info.showlegend = false;
publication_info.xlabel  = 'Noise Mask Level ($G$) ($\mathrm{dB}$)';
publication_info.ylabel  = 'Speech Intelligibility (STOI) (\%WC)';
publication_info.ylabel2 = 'Speech Quality (PESQ) (MOS)';

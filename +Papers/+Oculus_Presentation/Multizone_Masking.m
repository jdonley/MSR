...
for s = 1:numel(setup)
    ...
    setup(s).Multizone_Soundfield.QuietZ_Weight = 1000;
...
pk(1) = max(abs((setup(1).Bright_Samples(:))))*setup(1).res; % Source
pk(2) = max(abs((setup(2).Bright_Samples(:))))*setup(2).res; % Masker
...
fH = figure(111);
ha = tightPlots( 1, 1, ...
    SYS.publication_info.figure_width*1.2, ...
...
axes(ha(1));
ax=gca;
setup(1).plotSoundfield( Z1*db2mag(5) + Z2*db2mag(-5), 'scientific_D1', realistic, details);

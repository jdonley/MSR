function Broadband_PrivacyWeighted_v2( Input_file_path, Input_file_name, Input_file_ext)
...
Masker_LoudnessLevel = zeros( size(SPL_values, 1), 1);
for frame = 1:frames
    Masker_LoudnessLevel(frame) = mean( SPL_values(frame, :) );
end
Masker_LoudnessLevel = 2 * mean( Masker_LoudnessLevel ) - Masker_LoudnessLevel;

Desired_SPL = zeros( size(SPL_values) );
for frame = 1:frames
    Desired_SPL(frame, :) = Perceptual_Tools.Loudness( Frequencies_ , Masker_LoudnessLevel(frame) );
end
...
fileID = fopen(['+Results\' Output_file_path_ext 'PESQ_Results_PrivacyWeighted_v2.csv'],'a');
...
fileID = fopen(['+Results\' Output_file_path_ext 'PESQ_MOS_Results_PrivacyWeighted_v2.csv'],'a');
...
fileID = fopen(['+Results\' Output_file_path_ext 'ASL_Results_PrivacyWeighted_v2.csv'],'a');

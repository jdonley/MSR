function Broadband_PrivacyWeighted_v4_ANC( Input_file_path, Input_file_name, Input_file_ext, Noise_Mask_dB)
...
for ANC_Step = 1:2
    ...
    if ANC_Step == 1
        load(['+Soundfield_Database\+' num2str(speaker_radius*2) 'm_SpkrDia\+' num2str(loudspeakers) 'Spkrs_' num2str(speaker_arc) 'DegArc\LUT_Weight_vs_Frequency_' num2str(angle_pw) 'deg_' LUT_resolution '_multipoint.mat']);
    elseif ANC_Step == 2
        load(['+Soundfield_Database\+' num2str(speaker_radius*2) 'm_SpkrDia\+' num2str(loudspeakers) 'Spkrs_' num2str(speaker_arc) 'DegArc\LUT_Weight_vs_Frequency_' num2str(angle_pw) 'deg_' LUT_resolution '_multipoint_zones_swapped.mat']);
    end    
    ...
    % inverted quiet zone result which should cancel the previous quite
    ...
    Input_Signal = Quiet_sig; % Parallel Active Noise Cancellation
